...
run() {
    ...
    set -a  # broadcast defined var as envvar to subroutine ON
        ...
        [ $? != 0 ] && (echo  "ERROR cannot load $AH/.env"; kill $$)
            ...
            cd $SH
                docker-compose  -p $CONTAINER_PROJECT  -f "$SH/docker-compose.yml"  up -d
                    echo
                    docker ps --format '{{.Names}} {{.Ports}} {{.Networks}}' | grep -E "$DB_CONTAINER_NAME|$DB_PORT|$DOCKER_NETWORK" --color=always
}
    run
